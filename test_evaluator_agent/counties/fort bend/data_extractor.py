...
for input_file in input_files:
    ...
    if len(addr_parts) >= 1:
        ...
        if address_parts and address_parts[0].isdigit():
            ...
            if address_parts[1].upper() in ["N", "S", "E", "W", "NE", "NW", "SE", "SW"]:
    ...
    if tax_table:
        ...
        for row in rows:
            ...
            try:
                tax_json["property_market_value_amount"] = float(cols[1].text.replace("$", "").replace(",", "")) + float(cols[2].text.replace("$", "").replace(",", "")) if cols[1].text.strip() != "N/A" and cols[2].text.strip() != "N/A" else None
            except:
